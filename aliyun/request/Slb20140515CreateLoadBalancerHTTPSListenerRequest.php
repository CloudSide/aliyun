...
class Slb20140515CreateLoadBalancerHTTPSListenerRequest
{
	...
	/** 
	 * 服务器上配置的cookie，只有在StickySession设置为on且StickySessionType设置为server时有效。长度限制为1-200，cookie必须遵守 RFC 2965。这意味着它只能包含 ASCII 英文字母数字字符，不能包含逗号、分号或空格，也不能以 $ 字符开头。
	 **/
	private $cookie;
...
用户设置此参数为’$_ip’时，SLB会使用各后端服务器的私网IP当做健康检查使用的Domain。
	 **/
	private $healthCheckDomain;
	
	/** 
	 * 健康检查正常的http状态码，多个http状态码间用”,”分割。
	 **/
...
取值：1-10
	...
	public function setHealthCheckHttpCode($healthCheckHttpCode)
	{
		...
		$this->apiParas["HealthCheckHttpCode"] = $healthCheckHttpCode;
	}
