...
{
	/** 
	 * 监听的带宽峰值，范围-1-1000Mbps。针对按固定带宽计费方式的公网类型实例，不同Listener上的Bandwidth分配的带宽峰值总和不能超出在创建SLB实例时设定的Bandwidth值，且不能将Listener上的Bandwidth设置为-1；针对按使用流量计费方式的公网类型实例，可以选择将Listener上的Bandwidth设置为-1，表示不限制带宽峰值。
	 **/
	private $bandwidth;
...
取值：1-65535
默认值：无
用户不指定此参数时，表示此次调用不修改此配置项，保持之前的配置。健康检查的端口，范围为1-65535。默认使用后端服务端口。
	 **/
...
注意：如果HCTimeout < Interval，则HCTimeout无效，超时时间为HealthCheckInterval。健康检查的连接超时时间，单位为秒，范围为1-50，默认5。
	 **/
...
取值：1-5（单位为秒）
默认值：无
用户不指定此参数时，表示此次调用不修改此配置项，保持之前的配置。健康检查的间隔，单位为秒，取值区间为1-5。默认2。
	 **/
...
取值：1-10
默认值：无
用户不指定此参数时，表示此次调用不修改此配置项，保持之前的配置。后端服务器从fail到success的连续健康检查成功次数，取值区间为1-10，默认值3。
	 **/
	private $healthyThreshold;
	
	/** 
	 * SLB实例前端使用的端口，范围为1-65535。
	 **/
...
取值：1-10
默认值：无
用户不指定此参数时，表示此次调用不修改此配置项，保持之前的配置。后端服务器从success到fail的连续健康检查失败次数，取值区间为1-10，默认值3。
	 **/
